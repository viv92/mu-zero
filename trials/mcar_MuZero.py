...
ep_returns = []
replay_buffer = [] # to store episode trajectory used for training
for ep in range(1, num_episodes):
    ...
    for it in range(training_iterations):
        muzero_train(replay_buffer)



# plot
fig = plt.figure()
plt.plot(ep_returns, color='blue', label='ep_return')
plt.plot(ep_avg_return[1:], color='red', label='ep_avg_return')
plt.legend()
plt.xlabel('episode')
plt.ylabel('ep return')
plt.savefig('mcar_MuZero_moreLayers_epsDecay='+str(eps_decay_step)+'_lr='+str(lr)+'_planIter='+str(planning_iterations)+'_trainIter='+str(training_iterations)+'_numEP='+str(num_episodes)+'.png')

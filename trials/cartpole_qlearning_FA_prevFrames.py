...
for ep in range(1, num_episodes):
    ...
    while(True):
        ...
        # loss
        loss = td_error ** 2
        all_td_losses.append(loss.item())
        print('td_loss: ', loss.item())
...
ax[0].plot(ep_undiscounted_returns, label='ep_undiscounted_returns')
ax[1].plot(all_td_losses, label='td_loss')
plt.savefig('cartpole_qlearning_prevFrames='+str(num_prev_frames)+'_epsDecay='+str(eps_decay_step)+'_lr='+str(lr)+'_numEP='+str(num_episodes)+'.png')
